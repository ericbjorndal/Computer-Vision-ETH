...
def EstimateEssentialMatrix(K, im1, im2, matches):
  ...
  normalized_kps1 = (K_inv @ MakeHomogeneous(im1.kps, ax=1).T).T
  normalized_kps2 = (K_inv @ MakeHomogeneous(im2.kps, ax=1).T).T
...
def TriangulatePoints(K, im1, im2, matches):
  ...
  indices1 = ((R1 @ points3D.T).T + t1)[:, 2] > 0

  im1_corrs = im1_corrs[indices1]
  im2_corrs = im2_corrs[indices1]
  points3D = points3D[indices1]

  # Filter points behind the second camera
  indices2 = ((R2 @ points3D.T).T + t2)[:, 2] > 0

  im1_corrs = im1_corrs[indices2]
  im2_corrs = im2_corrs[indices2]
  points3D = points3D[indices2]
...
def EstimateImagePose(points2D, points3D, K):  
  ...
  #normalized_points2D = HNormalize((np.linalg.inv(K) @ MakeHomogeneous(points2D, 1).T).T, 1)
...
def TriangulateImage(K, image_name, images, registered_images, matches):
  ...
  corrs = {}
  total_image_corrs = np.array([])
  total_pts = 0
  for registered_image in registered_images:
    registered_matches = GetPairMatches(registered_image, image_name, matches)
    points, registered_corrs, image_corrs = TriangulatePoints(K, images[registered_image], image, registered_matches)

    points3D = np.concatenate([points3D, points], axis=0)
    corrs[registered_image] = [registered_corrs, total_pts, total_pts + len(points)]

    total_image_corrs = np.hstack([total_image_corrs, image_corrs])

    total_pts += len(points)

  corrs[image_name] = [total_image_corrs, 0, total_pts]
