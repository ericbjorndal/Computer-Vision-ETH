...
class SegNetLite(nn.Module):
    ...
    def __init__(self, kernel_sizes=[3, 3, 3, 3], down_filter_sizes=[32, 64, 128, 256],
        ...
        for i in range(self.num_down_layers):
            layers_conv_down.append(nn.Conv2d(in_channels=input_size, out_channels=down_filter_sizes[i], kernel_size=kernel_sizes[i], padding=conv_paddings[i]))
        ...
        for i in range(self.num_down_layers):
            ...
            layers_conv_up.append(nn.Conv2d(in_channels=input_size, out_channels=up_filter_sizes[i], kernel_size=kernel_sizes[i], padding=conv_paddings[i]))
    ...
    def forward(self, x):
        ...
        for i in range(self.num_down_layers):
            ...
            ids_list.append(indices)

        # upsample
        for i in range(self.num_down_layers):
            x = self.layers_unpooling[i](x, ids_list[self.num_down_layers-i-1])
