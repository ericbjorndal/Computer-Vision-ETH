...
def grid_points(img, nPointsX, nPointsY, border):
    ...
    xPoints = np.linspace(border, w-border, nPointsX)
    yPoints = np.linspace(border, h-border, nPointsY)

    xv, yv = np.meshgrid(xPoints, yPoints)
    vPoints = np.dstack([xv, yv]).reshape(-1, 2)
    # TODO # DONE
    ...
    for i in range(len(vPoints)):
        ...
        for cell_y in range(-2, 2):
            for cell_x in range(-2, 2):
                ...
                theta = np.atan2(grad_y[start_y:end_y, start_x:end_x], grad_x[start_y:end_y, start_x:end_x])
                histo = np.histogram(theta, bins=nBins, range=(-np.pi, np.pi))
...
def bow_histogram(vFeatures, vCenters):
    ...
    # TODO
    numCenters = vCenters.shape[0]
    numFeatures = vFeatures.shape[0]

    indices = [] 
    for i in range(numFeatures):
        j = np.argmin(np.linalg.norm(vFeatures[i] - vCenters, axis=1))
        indices.append(j)
    histo = np.histogram(indices, bins=numCenters)[0]
    return histo
