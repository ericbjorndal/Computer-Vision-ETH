...
def filter_keypoints(img, keypoints, patch_size = 9):
    ...
    boundary = int(np.floor(patch_size / 2.0))
    h, w = img.shape # height, width
    keypoints = np.array([pt for pt in keypoints if (pt[0] > boundary and pt[0] < w - boundary) and (pt[1] > boundary and pt[1] < h - boundary)])
    return keypoints
